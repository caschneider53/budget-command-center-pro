...
export default function Home() {
  ...
  const [msgs, setMsgs] = useState([
    {ai:true, text:"Hi! I'm your AI Budget Coach. Your data is saved automatically — add transactions and I'll help you stay on track."},
  ])
  ...
  // Save data to KV whenever txns, goals, or budgets change
  const saveData = useCallback((newTxns, newGoals, newBudgets) => {
    setSaveStatus('saving')
    fetch('/api/data', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({ transactions: newTxns, goals: newGoals, budgets: newBudgets })
    })
    .then(() => {
      setSaveStatus('saved')
  ...
  function addTxn() {
    ...
    setTxns(newTxns)
    saveData(newTxns, goals, budgets)
    setModal(false)
  ...
  function updateGoal(idx, val) {
    ...
    setGoals(newGoals)
    saveData(txns, newGoals, budgets)
  }
  ...
  if (loading) return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'center',height:'100vh',background:'var(--bg)',color:'var(--muted)',fontFamily:'Inter,sans-serif',flexDirection:'column',gap:'12px'}}>
      ...
      <div style={{fontSize:'14px'}}>Loading your budget...</div>
    </div>
  )
  ...
  return (
    <>
              ...
              <>
                <div style={S.kgrid}>
                  ...
                  <div style={S.kpi}>
                    ...
                    <div style={S.kmeta(parseFloat(savingsRate)>0)}>{income===0?'Log income first':parseFloat(savingsRate)>15?'Great rate!':'Keep going'}</div>
                  </div>
                </div>
                <div style={S.g2}>
                  <div style={S.card}>
            ...
            {tab==='Budget' && (
              <>
                <div style={S.kgrid}>
                  ...
                  <div style={S.kpi}><div style={S.klabel}>Remaining</div><div style={S.kval}>{fmt(budgets.reduce((s,c)=>s+c.budget,0)-spent)}</div><div style={S.kmeta(true)}>Budget left</div></div>
                </div>
                <div style={S.card}>
                    ...
                    <thead>
                      <tr style={{borderBottom:'1px solid var(--border)'}}>
                        {['Date','Description','Category','Amount','Type',''].map((h,i)=>(
                          <th key={i} style={{textAlign:'left',padding:'8px 10px',fontSize:'10px',textTransform:'uppercase',letterSpacing:'0.09em',color:'var(--muted)',fontWeight:600}}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {txns.map(t=>(
                        <tr key={t.id} style={{borderBottom:'1px solid var(--border)',transition:'background 120ms'}}
                          ...
                          <td style={{padding:'10px'}}>
                            <button onClick={()=>deleteTxn(t.id)} style={{color:'var(--muted)',fontSize:'14px',padding:'2px 6px',borderRadius:'4px',transition:'color 120ms'}}
                    ...
                    <div key={g.id||gi} style={{...S.card,display:'flex',flexDirection:'column',gap:'13px'}}>
                      ...
                      <div>
                        <div style={{display:'flex',justifyContent:'space-between',marginBottom:'5px',fontSize:'12px'}}>
                          ...
                          <span style={S.tag(`var(--${g.color})`)}>{pct}%</span>
                        </div>
                      ...
                      <input
                        ...
                        type="number"
                        placeholder="Update saved amount & press Enter..."
                        onKeyDown={e=>{
                          if(e.key==='Enter'&&e.target.value){
                            const val=parseFloat(e.target.value)
                            if(!isNaN(val)&&val>=0){updateGoal(gi,val);e.target.value=''}
                          }
                        }}
                      />
      ...
      {saveStatus && (
        <div style={S.saving}>
          {saveStatus==='saving' ? <><span style={{fontSize:'10px'}}>⏳</span> Saving...</> : <><span style={{color:'var(--green)',fontSize:'12px'}}>✓</span> Saved</>}
        </div>
      )}
